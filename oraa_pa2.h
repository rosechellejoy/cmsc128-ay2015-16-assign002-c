...
int isValidString(char str[50], char alphabet[50]){
	int i, j;
	if(strlen(str)<=0 || strlen(alphabet)<=0) return -1; 
	for(i=0; i<strlen(str); i++){
		...
		if(j==strlen(alphabet)) return 0;		//if str[i] doesn't match with any character in alphabet, then str is invalid
	}
	return 1;
}
/*
